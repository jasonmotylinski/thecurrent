...
class SaveDayJsonToLocal(luigi.Task):
    ...
    def output(self):
        """Output."""
        return luigi.LocalTarget(config.KEXP_DAY_JSON.format(year=int(self.date.strftime("%Y")), month=int(self.date.strftime("%m")), day=int(self.date.strftime("%d"))))

    def run(self):
        """Run."""
        with self.output().open('w') as f:
            r=requests.get('https://api.kexp.org/v2/plays/?limit=1000&airdate_after={date.year}-{date.month}-{date.day}T00:00:00&airdate_before={date.year}-{date.month}-{date.day}T23:59:59'.format(date=self.date))
