...
class SaveDayJsonToLocal(luigi.Task):
    ...
    def output(self):
        """Output."""
        return luigi.LocalTarget(config.KEXP.DAY_JSON.format(year=int(self.date.strftime("%Y")), month=int(self.date.strftime("%m")), day=int(self.date.strftime("%d"))))

    def run(self):
        """Run."""
        with self.output().open('w') as f:
            r=requests.get(config.KEXP.DAY_URL.format(date=self.date))
